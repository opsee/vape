...
func (s *ApiSuite) TestUserGet(c *C) {
	...
	rec, err := testAuthedReq(&schema.User{Id: 1, Email: "cliff@leaninto.it", Admin: false, Perms: nonAdminPerms, TeamFlags: teamFlags}, "GET",
	...
	rec, err = testAuthedReq(&schema.User{Id: 2, Email: "cliff@leaninto.it", Admin: false, Perms: nonAdminPerms, TeamFlags: teamFlags}, "GET",
		"https://vape/users/1", nil, nil)
	if err != nil {
		c.Fatal(err)
	}
	...
	rec, err = testAuthedReq(&schema.User{Id: 2, Email: "cliff@leaninto.it", Admin: true, Perms: adminPerms, TeamFlags: teamFlags}, "GET",
		"https://vape/users/1", nil, nil)
	if err != nil {
		c.Fatal(err)
	}
...
func (s *ApiSuite) TestUserUpdate(c *C) {
	...
	rec, err := testAuthedReq(&schema.User{Id: 1, Email: "cliff@leaninto.it", Admin: false, Perms: nonAdminPerms, TeamFlags: teamFlags}, "PUT",
...
func (s *ApiSuite) TestGetListUsers(c *C) {
	...
	// get list, fail
	rec, _ := testAuthedReq(&schema.User{Id: 1, Email: "cliff@leaninto.it", Admin: false, Perms: nonAdminPerms, TeamFlags: teamFlags}, "GET", "https://vape/users/", nil, nil)
	messageResponse := &MessageResponse{}
	loadResponse(messageResponse, rec.Body)
	c.Assert(messageResponse.Message, DeepEquals, Messages.UserOrAdminRequired)

	// get list
	rec, _ = testAuthedReq(&schema.User{Id: 1, Email: "cliff@leaninto.it", Admin: true, Perms: adminPerms, TeamFlags: teamFlags}, "GET", "https://vape/users/", nil, nil)
